 ...
 * M210 - When the timing belt is driven, there is a gap between the belt and the synchronous wheel; or when the Z-axis moves, 
          there is also a gap between the screw and the nut. This can lead to errors in commutation. 
          The M210 command set a compensation to correct this gap. "M210 X<gap> Y<gap> Z<gap>"
...
#if ENABLED(ADJUST_MIN_POS_MENU)
   float adj_min_pos[XYZ] = { 0 };
#endif
...
#if HAS_AUTO_FAN && ENABLED(ADJUST_ATUO_FAN_SPEED)
uint8_t auto_fan_speed = EXTRUDER_AUTO_FAN_SPEED;
#endif
...
float cartes[XYZ] = { 0 };
  ...
  static bool old_filament_ran_out = false;
#endif
  ...
  float auto_mixing_E2percent = 1.0;
  #endif
#endif
...
static void set_axis_is_at_home(const AxisEnum axis) {
  ...
  {
    #if ENABLED(ADJUST_MIN_POS_MENU)
	current_position[axis] = base_home_pos(axis) + adj_min_pos[axis];
	#else
...
inline void gcode_M205() {
  ...
  /**
   * M210: set compensation to correct this gap
   *
   *	X = Correction of X Gap (steps), 0~255
   *	Y = Correction of Y Gap (steps), 0~255
   *	Z = Correction of Z Gap (steps), 0~255
   */
  inline void gcode_M210() {
    LOOP_XYZ(i)
      if (parser.seen(axis_codes[i]))
	  	planner.gap_correction[i] = parser.value_byte();
  }

#endif // ENABLED(GAP_CORRECTION)
...
void quickstop_stepper() {
  ...
  inline void gcode_M428() {
    ...
    LOOP_XYZ(i) {
      ...
      if (!WITHIN(diff[i], -100, 100) && home_dir((AxisEnum)i) > 0)
        diff[i] = current_position[i];
	  else
	  	diff[i] = -current_position[i];
      if (!WITHIN(diff[i], -100, 100)) {
        SERIAL_ERROR_START();
...
void normalize_mix() 
{
  ...
  inline void gcode_M163() {
    ...
    inline void gcode_M169() {
	  ...
	  if(parser.seenval('K'))
	  	auto_mixing_ext2percent = parser.value_float();
	  ...
	  auto_mixing_E2percent = auto_mixing_ext2percent;
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
	  ...
	  #if HAS_AUTO_FAN && ENABLED(ADJUST_ATUO_FAN_SPEED)
        case 116: // M116: Set cooling FAN Speed
          gcode_M116();
          break;
      #endif
...
void prepare_move_to_destination() {
  ...
  void handle_filament_runout() {
    if (!filament_ran_out) {
	  ...
	  SERIAL_PROTOCOLLNPGM("Catch filament run out!");
...
#define	AUTO_MIX_MODE1		1
#define	AUTO_MIX_MODE2		2
#define	AUTO_MIX_MODE3		3
#define	AUTO_MIX_MODE4		4
...
void auto_mixing_preocess(uint8_t percent){
	...
	if((auto_mixing_mode > 0) && (percent - old_progress_bar_percent > 0))
	{
		...
		switch(auto_mixing_mode)
		{
			...
			#else
			mixing_factor_buf[0] = ((auto_mixing_max - auto_mixing_min)*(100-percent)/100) + auto_mixing_min;
			mixing_factor_buf[1] = (uint8_t)(((float)(100 - mixing_factor_buf[0]))*auto_mixing_E2percent);
			mixing_factor_buf[2] = (100 - mixing_factor_buf[0] - mixing_factor_buf[1]);
			#endif
			normalize_mix();
			break;
			...
			//From EXT1-->EXT2-->EXT3
			if(percent < 50){
				mixing_factor_buf[0] = (100-percent*2);
				mixing_factor_buf[1] = (100-mixing_factor_buf[0]);
				mixing_factor_buf[2] = 0;
			}
			else{
				mixing_factor_buf[0] = 0;
				mixing_factor_buf[1] = (100-(percent-50)*2);
				mixing_factor_buf[2] = (100-mixing_factor_buf[1]);
			}			
			normalize_mix();		
			break;
			#endif			
...
#define IS_FIL_RUN_OUT (READ(FIL_RUNOUT_PIN) == FIL_RUNOUT_INVERTING)
  ...
  #if ENABLED(FILAMENT_RUNOUT_SENSOR)    
	#if 1//Filament run out modify
    if((IS_SD_PRINTING) || print_job_timer.isRunning())
	{
		//printing
	    if(IS_FIL_RUN_OUT)		
        {
          if(!old_filament_ran_out){
		  	old_filament_ran_out = true;
			handle_filament_runout();
          }
        }
	    else 
		  old_filament_ran_out = false;
	}
	else
    {    	
    	//idel
        if(IS_FIL_RUN_OUT){
		  ...
		  if(old_filament_ran_out)
		  	SERIAL_PROTOCOLLNPGM("Filament insert!");
		  old_filament_ran_out = false;
	    }
    }	
	#else
...
void setup() {
  ...
  #if(MODEL_NUMBER == M809)
  OUT_WRITE(LOGO_LED_PIN, HIGH);
  OUT_WRITE(BOARD_COOL_PIN, HIGH);
  #endif
