...
volatile signed char Stepper::old_count_direction[NUM_AXIS] = { 1, 1, 1, 1 };
#endif
...
void Stepper::isr() {
  ...
  if (!current_block) {
	...
	#if ENABLED(GAP_CORRECTION)
	  if(old_count_direction[X_AXIS] != count_direction[X_AXIS]){
		  current_block->steps[X_AXIS] +=  planner.gap_correction[X_AXIS];
		  old_count_direction[X_AXIS] = count_direction[X_AXIS];
	  }
	  if(old_count_direction[Y_AXIS] != count_direction[Y_AXIS]){
		  current_block->steps[Y_AXIS] +=  planner.gap_correction[Y_AXIS];
		  old_count_direction[Y_AXIS] = count_direction[Y_AXIS];
	  }
	  if(old_count_direction[Z_AXIS] != count_direction[Z_AXIS]){
		  current_block->steps[Z_AXIS] +=  planner.gap_correction[Z_AXIS];
		  old_count_direction[Z_AXIS] = count_direction[Z_AXIS];
	  }	  
	#endif
