 ...
 *
 * GAP_CORRECTION:                                 3 bytes
 *  726  M210 XYZ  planner.gap_correction       	(uint8_t) 
 *  729  M211 XYZ  adj_min_pos                      (float x3) 
 *
 *  741                                   Minimum end-point
 * 2255 (741 + 208 + 36 + 9 + 288 + 988)  Maximum end-point
 *
 * ========================================================================
...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::save() {
    ...
    //
      ...
      EEPROM_WRITE(planner.gap_correction);      
    #else
      uint8_t dummygap = 0;
      for (uint8_t q = 3; q--;) EEPROM_WRITE(dummygap);
    #endif

	#if ENABLED(ADJUST_MIN_POS_MENU)
	  EEPROM_WRITE(adj_min_pos);
	#else	  
	  const float adj_min_pos[XYZ] = { 0 };
	#endif
  ...
  bool MarlinSettings::load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
	  #if ENABLED(GAP_CORRECTION)
        EEPROM_READ(planner.gap_correction);      
      #else
        for (uint8_t q = 3; q--;) EEPROM_READ(dummyui8);
      #endif

	  #if ENABLED(ADJUST_MIN_POS_MENU)
      float adj_min_pos[XYZ] = {0,0,0};
      #endif
//      EEPROM_READ(adj_min_pos);
...
void MarlinSettings::reset() {
  ...
  void MarlinSettings::report(const bool forReplay) {
     ...
     */
      ...
      if (!forReplay) {
        CONFIG_ECHO_START;
        SERIAL_ECHOLNPGM("XYZ Gap Correction: ");
      }
      CONFIG_ECHO_START;
      SERIAL_ECHO("  M210");
      SERIAL_ECHOLNPAIR(" X", planner.gap_correction[X_AXIS]);
	  SERIAL_ECHOLNPAIR(" Y", planner.gap_correction[Y_AXIS]);
	  SERIAL_ECHOLNPAIR(" Z", planner.gap_correction[Z_AXIS]);
	  SERIAL_EOL();
    #endif
      ...
      if (!forReplay) {
        CONFIG_ECHO_START;
        SERIAL_ECHOLNPGM("Min Pos Adj:");
      }
      CONFIG_ECHO_START;
      SERIAL_ECHOPAIR("  M211 X", LINEAR_UNIT(adj_min_pos[X_AXIS]));
      SERIAL_ECHOPAIR(" Y", LINEAR_UNIT(adj_min_pos[Y_AXIS]));
      SERIAL_ECHOLNPAIR(" Z", LINEAR_UNIT(adj_min_pos[Z_AXIS]));
	#endif
