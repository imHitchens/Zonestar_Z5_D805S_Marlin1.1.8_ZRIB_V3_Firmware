...
void kill_screen(const char* lcd_msg) {
  ...
  void lcd_main_menu() {
    ...
    #if HAS_SUICIDE
	  MENU_ITEM(gcode, MSG_POWERDOWN, PSTR("M81")); // Manually initialize the SD-card via user interface
    #endif
  ...
  void lcd_set_mix_ratio(){
    ...
    void lcd_set_auto_mix_mode(){
		START_MENU();
    	MENU_BACK(MSG_TUNE);
		MENU_ITEM_EDIT_CALLBACK(int8, MSG_SET_AUTO_MIXRATE, &auto_mixing_mode, 0,MIXING_STEPPERS,set_mixing_mode_callback);
		if(auto_mixing_mode == 1){
			MENU_ITEM_EDIT_CALLBACK(int8, MSG_SET_AUTOMIX_E1MIN, &auto_mixing_min, 0, 100, set_mixing_mode_callback);
			MENU_ITEM_EDIT_CALLBACK(int8, MSG_SET_AUTOMIX_E1MAX, &auto_mixing_max, 0, 100, set_mixing_mode_callback);
			#if (MIXING_STEPPERS > 2)
			MENU_ITEM_EDIT_CALLBACK(float52, MSG_SET_AUTOMIX_E2PERCENT, &auto_mixing_E2percent, 0.1, 10.0, set_mixing_mode_callback);
			#endif
		}
    	END_MENU();
    }
  ...
  void lcd_cooldown() {
    ...
    static void lcd_store_settings()   { lcd_completion_feedback(settings.save()); lcd_return_to_status();}
    ...
    void _lcd_goto_next_corner() {	  
      ...
      switch (bed_corner) {
        case 0:
          current_position[X_AXIS] = adj_min_pos[X_AXIS] + X_MIN_BED + DISTANCE_TO_CORNER_X;
          current_position[Y_AXIS] = adj_min_pos[Y_AXIS] + Y_MIN_BED + DISTANCE_TO_CORNER_X;
          break;
        case 1:
  ...
  void lcd_prepare_menu() {
    ...
    // Switch power on/off
    //
    #if HAS_POWER_SWITCH && (MODEL_NUMBER == M809)
      if (powersupply_on)
  ...
  static void lcd_callback_set_minendstop(){
	  	#if ENABLED(EEPROM_SETTINGS)
		 settings.save();
		#endif
  	}
  ...
  void lcd_control_temperature_menu() {
	...
	MENU_ITEM_EDIT(int8, MSG_COOL_FAN_SPEED, &auto_fan_speed, 64, 255);	
    #endif
  ...
  void lcd_control_motion_menu() {
	...
	#if ENABLED(ADJUST_MIN_POS_MENU)
	MENU_ITEM_EDIT(float32, MSG_HOME_X_OFFSET, &adj_min_pos[X_AXIS], -30, 30);
	MENU_ITEM_EDIT(float32, MSG_HOME_Y_OFFSET, &adj_min_pos[Y_AXIS], -30, 30);
	MENU_ITEM_EDIT(float32, MSG_HOME_Z_OFFSET, &adj_min_pos[Z_AXIS], -10, 10);
    #endif
	...
	//Dual Z offset
	#if ENABLED(Z_DUAL_STEPPER_DRIVERS) && ENABLED(Z_DUAL_ENDSTOPS)
    ...
    // M540 S - Abort on endstop hit when SD printing
	  ...
	  MENU_ITEM_EDIT(int8, MSG_GAP_CORRECTION_X, &planner.gap_correction[X_AXIS], 0, 100);
	  MENU_ITEM_EDIT(int8, MSG_GAP_CORRECTION_Y, &planner.gap_correction[Y_AXIS], 0, 100);
	  MENU_ITEM_EDIT(int8, MSG_GAP_CORRECTION_Z, &planner.gap_correction[Z_AXIS], 0, 100);
	#endif
